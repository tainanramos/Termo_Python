    ...
    def output_result(self, termo_row):
        ...
        for i in range(0,5):
            ...
            if "wrong" in class_termo:
                table_temp = pd.DataFrame({"POSITION":[i], "CLASS":["wrong"], "LETTER":[unidecode(str(wc_row.text)).lower()]})
                table_result = pd.concat([table_result, table_temp])
            elif "place" in class_termo:
                table_temp = pd.DataFrame({"POSITION":[i], "CLASS":["place"], "LETTER":[unidecode(str(wc_row.text)).lower()]})
                table_result = pd.concat([table_result, table_temp])
            elif "right" in class_termo:
                table_temp = pd.DataFrame({"POSITION":[i], "CLASS":["right"], "LETTER":[unidecode(str(wc_row.text)).lower()]})
                table_result = pd.concat([table_result, table_temp])
                count_right += 1
