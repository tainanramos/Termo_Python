...
class Words:
    def __init__(self, strPath):
        ...
        self.words = pd.read_excel(strPath)

    
    def filter_by_size(self, intTamanho):
        print(f"FILRANDO PALAVRAS DE {intTamanho} LETRAS...")
        self.words = self.words.query(f"TAMANHO == {intTamanho}")
        self._re_index()

    
    def filter_by_class(self, table_result):
        for index, row in table_result.iterrows():
            if (row["CLASS"] == "wrong") & (table_result.query(f"LETTER == '{row['LETTER']}' & (CLASS == 'right' | CLASS == 'place')").shape[0] == 0):
                self.words = self.words[~self.words["PALAVRA"].str.contains(row["LETTER"])]
                self._re_index()
            elif row["CLASS"] == "place":
                self.words = self.words[(self.words["PALAVRA"].str.contains(row["LETTER"])) & (self.words["PALAVRA"].str[int(row["POSITION"])] != row["LETTER"])]
                self._re_index()
